...
namespace ASP_TASK_7
{
    public partial class Task_7 : System.Web.UI.Page
    {
        ...
        public void ViewButton_Click(object sender, EventArgs e)
        {
            try
            {
                DataBaseHelper.MonthYear.selectedYear = Convert.ToInt32(year.Text);
                DataBaseHelper.MonthYear.selectedMonth = Convert.ToInt32(month.Text);
                ...
                int daysInMonth = DateTime.DaysInMonth(DataBaseHelper.MonthYear.selectedYear, DataBaseHelper.MonthYear.selectedMonth);
                ...
                if (dt_frontend != null && dt_frontend.Rows.Count > 0)
                {
                    ...
                    for (int j = 6; j < dt_frontend.Columns.Count; j++)
                    {
                        ...
                        else
                        {
                            ...
                            if (approvalRow != null)
                            {
                                value = approvalRow["OperatorID"].ToString();
                                operatorID = $"<span style='color:black;'>{value}</span>";
                    ...
                    for (int i = 2; i < list.Count; i++) // Index starts from 0, so 2 is the 3rd row
                    {
                        var row = list[i];
                        var innerList = row.innerlistviewdata;

                        int totalRows = list.Count;
                        int middleRowIndex = totalRows / 2;

                        foreach (var data in innerList)
                        {
                            if (data.dynamic_dates != "OK")
                            {
                                if (i == middleRowIndex)
                                {
                                    // Middle row with specific styling
                                    data.dynamic_dates = $"<span style='color:black; border:none;'>{data.dynamic_dates}</span>";
                                }
                                else
                                {
                                    // Other rows with hidden styling
                                    data.dynamic_dates = $"<span style='color:white; border:none;'>{data.dynamic_dates}</span>";
                                }
                            }
                        }
                    }
